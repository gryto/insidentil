...
class PDFHomePage2 extends StatefulWidget {
  String lapinsusCreated,
   lapinsusId;
   List data;
  PDFHomePage2({
    super.key,
  ...
  @override
  State<PDFHomePage2> createState() => _PDFHomePage2State();
}

class _PDFHomePage2State extends State<PDFHomePage2> {
  String? pdfPath;
  ...
  Future<void> fetchDataAndGeneratePDF(String reportId) async {
    ...
    var report = widget.data.firstWhere((report) => report['id'].toString() == reportId);
  ...
  Future download2(context, fileUrl, savePath) async {
    try {
      ...
      _onAlertButtonPressed(context, true, "File PDF berhasil di download di folder internal storage /Download");
    } catch (e) {
